import { e as V } from "./index.modern.BC8Oj8jT.js";
import { l as p, n as P } from "./Swup.Cr7ogLqN.js";
function w() {
  return (
    (w = Object.assign
      ? Object.assign.bind()
      : function (d) {
          for (var e = 1; e < arguments.length; e++) {
            var o = arguments[e];
            for (var s in o) ({}).hasOwnProperty.call(o, s) && (d[s] = o[s]);
          }
          return d;
        }),
...
function b(d) {
  return !!d && (d instanceof HTMLAnchorElement || d instanceof SVGAElement);
}
const E = window.requestIdleCallback || ((d) => setTimeout(d, 1)),
...
class A extends V {
  constructor(e = {}) {
    ...
    super(),
      ...
      (this.onMouseEnter = async function (t) {
        if (t.target !== t.delegateTarget || !L()) return;
        const i = t.delegateTarget;
        if (!b(i)) return;
        const { url: a, hash: n } = p.fromElement(i),
          u = o.swup.createVisit({ to: a, hash: n, el: i, event: t });
        o.swup.hooks.callSync("link:hover", u, { el: i, event: t }),
          o.preload(i, { priority: !0 });
      }),
      (this.onTouchStart = (t) => {
        if (L()) return;
        const i = t.delegateTarget;
        b(i) && this.preload(i, { priority: !0 });
      }),
      (this.onFocus = (t) => {
        const i = t.delegateTarget;
        b(i) && this.preload(i, { priority: !0 });
    ...
    (this.options = w({}, this.defaults, r)),
      typeof s == "object"
        ? (this.options.preloadVisibleLinks = w(
            {},
          ...
          add: function (f, m = !1) {
            if (f.__queued) {
              if (!m) return;
              {
                const g = i.indexOf(f);
                if (g >= 0) {
                  const h = i.splice(g, 1);
                  n -= h.length;
                }
              }
            }
            (f.__queued = !0), (m ? a : i).push(f), n++, n <= 1 && c();
          },
          next: function () {
            u--, c();
          },
  ...
  unmount() {
    ...
    (this.swup.preload = void 0),
      ...
      (o = this.touchStartDelegate) == null || o.destroy(),
  ...
  async preload(e, o = {}) {
    var s;
    let r, t;
    const i = (s = o.priority) != null && s;
    if (Array.isArray(e)) return Promise.all(e.map((n) => this.preload(n)));
    if (b(e)) (t = e), ({ href: r } = p.fromElement(e));
    else {
      if (typeof e != "string") return;
      r = e;
    }
    if (!r) return;
    if (this.swup.cache.has(r)) return this.swup.cache.get(r);
    if (this.preloadPromises.has(r)) return this.preloadPromises.get(r);
    if (!this.shouldPreload(r, { el: t })) return;
    ...
    return this.preloadPromises.set(r, a), a;
  }
  preloadLinks() {
    E(() => {
      Array.from(
  ...
  preloadLinksOnAttention() {
    ...
    (this.mouseEnterDelegate = e.delegateEvent(
      ...
      (this.focusDelegate = e.delegateEvent(o, "focus", this.onFocus, s));
  }
  preloadVisibleLinks() {
    ...
    (this.preloadObserver = (function ({
      ...
      const u = new Map(),
        c = new IntersectionObserver(
          (h) => {
            h.forEach((l) => {
              l.isIntersecting ? f(l.target) : m(l.target);
            });
          },
          { threshold: r }
        ),
        f = (h) => {
          var l;
          const { href: v } = p.fromElement(h),
            y = (l = u.get(v)) != null ? l : new Set();
          u.set(v, y),
            y.add(h),
            setTimeout(() => {
              const k = u.get(v);
              k != null && k.size && (a(h), c.unobserve(h), k.delete(h));
            }, t);
        },
        m = (h) => {
          var l;
          const { href: v } = p.fromElement(h);
          (l = u.get(v)) == null || l.delete(h);
        },
        g = () => {
          E(() => {
            const h = i.map((l) => `${l} a[*|href]`).join(", ");
            Array.from(document.querySelectorAll(h))
              .filter((l) => n(l))
              .forEach((l) => c.observe(l));
          });
        };
      return {
        start: () => g(),
      ...
      delay: o,
      containers: s,
      callback: (r) => this.preload(r),
      filter: (r) => {
        if (
          this.options.preloadVisibleLinks.ignore(r) ||
          !r.matches(this.swup.options.linkSelector)
        )
          return !1;
        const { href: t } = p.fromElement(r);
        return this.shouldPreload(t, { el: r });
      },
    })),
  ...
  shouldPreload(e, { el: o } = {}) {
    const { url: s, href: r } = p.fromUrl(e);
    return !(
      !(function () {
        if (navigator.connection) {
          ...
          if (
            ...
            ((t = navigator.connection.effectiveType) != null &&
              t.endsWith("2g"))
          )
            return !1;
        }
      ...
      this.swup.shouldIgnoreVisit(r, { el: o }) ||
      (o && this.swup.resolveUrl(s) === this.swup.resolveUrl(P()))
    );
  }
  async performPreload(e) {
    var o = this;
    const { url: s } = p.fromUrl(e),
    ...
    return await this.swup.hooks.call(
      ...
      { url: s },
      async function (i, a) {
        return (a.page = await o.swup.fetchPage(e, { visit: i })), a.page;
      }
    );
  }
}
