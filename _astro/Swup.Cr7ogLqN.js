...
function x(e, t, i, s) {
  if (!e && !$.has(t)) return !1;
  const n = $.get(t) ?? new WeakMap();
  $.set(t, n);
  const o = n.get(i) ?? new Set();
  n.set(i, o);
  const r = o.has(s);
  return e ? o.add(s) : o.delete(s), r && e;
}
function R(e, t) {
  let i = e.target;
  if (
...
function D(e, t, i, s = {}) {
  ...
  const { once: r, ...a } = s,
    ...
    c = !!(typeof s == "object" ? s.capture : s),
    h = (m) => {
      const g = R(m, String(e));
      if (g) {
        const f = Object.assign(m, { delegateTarget: g });
        i.call(l, f), r && (l.removeEventListener(t, h, a), x(!1, l, i, u));
      }
    },
    u = JSON.stringify({ selector: e, type: t, capture: c });
  x(!0, l, i, u) || l.addEventListener(t, h, a),
...
async function B(e, t = {}) {
  ...
  try {
    (l = await this.hooks.call(
      ...
      s,
      { url: e, options: t },
      (w, { url: k, options: y }) => fetch(k, y)
    )),
      h && clearTimeout(h);
  } catch (w) {
    throw c
      ? (this.hooks.call("fetch:timeout", s, { url: e }),
        new E(`Request timed out: ${e}`, { url: e, timedOut: c }))
      : w?.name === "AbortError" || a.aborted
  ...
  const { url: g } = v.fromUrl(d),
    f = { url: g, html: m };
  return (
    !s.cache.write ||
      (t.method && t.method !== "GET") ||
      e !== g ||
      this.cache.set(f.url, f),
    f
  );
}
class _ {
  ...
  resolve(t) {
    const { url: i } = v.fromUrl(t);
    return this.swup.resolveUrl(i);
  }
}
const L = (e, t = document) => t.querySelector(e),
  H = (e, t = document) => Array.from(t.querySelectorAll(e)),
  q = () =>
    new Promise((e) => {
...
class M {
  constructor(t, i) {
    (this.id = void 0),
      ...
      (this.scroll = void 0),
      (this.meta = void 0);
    const { to: s, from: n, hash: o, el: r, event: a } = i;
    (this.id = Math.random()),
      (this.state = 1),
...
class G {
  ...
  async call(t, i, s, n) {
    const [o, r, a] = this.parseCallArgs(t, i, s, n),
      { before: l, handler: c, after: h } = this.getHandlers(t, a);
    await this.run(l, o, r);
    const [u] = await this.run(c, o, r, !0);
    return await this.run(h, o, r), this.dispatchDomEvent(t, o, r), u;
  }
  callSync(t, i, s, n) {
    const [o, r, a] = this.parseCallArgs(t, i, s, n),
      { before: l, handler: c, after: h } = this.getHandlers(t, a);
    this.runSync(l, o, r);
    const [u] = this.runSync(c, o, r, !0);
    return this.runSync(h, o, r), this.dispatchDomEvent(t, o, r), u;
  }
  parseCallArgs(t, i, s, n) {
    return i instanceof M || (typeof i != "object" && typeof s != "function")
      ? [i, s, n]
      : [void 0, i, s];
  }
  async run(t, i = this.swup.visit, s, n = !1) {
    const o = [];
    for (const { hook: r, handler: a, defaultHandler: l, once: c } of t)
      if (i == null || !i.done) {
        c && this.off(r, a);
        try {
          const h = await F(a, [i, s, l]);
          o.push(h);
        } catch (h) {
          if (n) throw h;
          console.error(`Error in hook '${r}':`, h);
        }
      }
    return o;
  }
  ...
  getHandlers(t, i) {
    ...
    const n = Array.from(s.values()),
      ...
      r = n.filter(({ before: u, replace: d }) => u && !d).sort(o),
      a = n
        .filter(({ replace: u }) => u)
        .filter((u) => !0)
        .sort(o),
      ...
      c = a.length > 0;
    let h = [];
    if (i && ((h = [{ id: 0, hook: t, handler: i }]), c)) {
      const u = a.length - 1,
        { handler: d, once: m } = a[u],
        g = (f) => {
          const w = a[f - 1];
          return w ? (k, y) => w.handler(k, y, g(f - 1)) : i;
        };
      h = [{ id: 0, hook: t, once: m, handler: d, defaultHandler: g(u) }];
    }
    return { found: !0, before: r, handler: h, after: l, replaced: c };
  }
  sortRegistrations(t, i) {
    var s, n;
    return (
      ((s = t.priority) != null ? s : 0) - ((n = i.priority) != null ? n : 0) ||
      t.id - i.id ||
      0
    );
  }
...
const J = (e) => {
    ...
    let i =
      ...
      L(`a[name='${CSS.escape(e)}']`) ||
      L(`a[name='${CSS.escape(t)}']`);
...
async function X({ selector: e, elements: t }) {
  ...
  let i = [];
  if (t) i = Array.from(t);
  else if (e && ((i = H(e, document.body)), !i.length))
    ...
    (function (o) {
      const {
        ...
        return {
          type: y,
          timeout: k,
          propCount: y ? (y === C ? d.length : f.length) : 0,
        };
      })(o);
      return (
        !(!r || !a) &&
        new Promise((c) => {
          ...
          let d = 0;
          const m = () => {
              o.removeEventListener(h, g), c();
            },
            g = (f) => {
              f.target === o &&
                ((performance.now() - u) / 1e3 < f.elapsedTime ||
                  (++d >= l && m()));
            };
          setTimeout(() => {
            d < l && m();
          }, a + 1),
            o.addEventListener(h, g);
        })
      );
    })(n)
  );
...
function Q(e, t = {}, i = {}) {
  ...
  const { url: s, hash: n } = v.fromUrl(e),
  ...
  typeof n == "string" && (e.animation.name = n),
    ...
    typeof t.cache == "object"
      ? ((e.cache.read = (o = t.cache.read) != null ? o : e.cache.read),
        (e.cache.write = (r = t.cache.write) != null ? r : e.cache.write))
      : t.cache !== void 0 && (e.cache = { read: !!t.cache, write: !!t.cache }),
    delete t.cache;
  try {
    await this.hooks.call("visit:start", e, void 0), (e.state = 3);
    const a = this.hooks.call("page:load", e, { options: t }, async (c, h) => {
      let u;
      return (
    ...
    a.then(({ html: c }) => {
      e.advance(5),
        (e.to.html = c),
        (e.to.document = new DOMParser().parseFromString(c, "text/html"));
    });
    const l = e.to.url + e.to.hash;
    if (
      (e.history.popstate ||
        (e.history.action === "replace" || e.to.url === this.location.url
          ? b(l)
          : (this.currentHistoryIndex++,
            j(l, { index: this.currentHistoryIndex }))),
          ...
          e.advance(4),
            ...
            e.animation.native && document.startViewTransition
              ? await document.startViewTransition(
                  async () => await this.renderPage(e, await a)
                ).finished
      ...
      return;
    await this.hooks.call("visit:end", e, void 0, () => this.classes.clear()),
      (e.state = 7),
      (this.navigating = !1),
...
const Z = async function (e) {
  ...
  tt = function (e) {
    ...
    document.title = s;
    const n = H('[data-swup-persist]:not([data-swup-persist=""])'),
      o = e.containers
        .map((r) => {
          const a = document.querySelector(r),
            l = i.querySelector(r);
          return a && l
            ? (a.replaceWith(l.cloneNode(!0)), !0)
            : (a ||
                console.warn(
                  `[swup] Container missing in current document: ${r}`
                ),
              l ||
                console.warn(
                  `[swup] Container missing in incoming document: ${r}`
                ),
              !1);
        })
        .filter(Boolean);
    return (
      n.forEach((r) => {
        const a = r.getAttribute("data-swup-persist"),
          l = L(`[data-swup-persist="${a}"]`);
        l && l !== r && l.replaceWith(r);
      }),
    ...
    return (
      ...
      s &&
        ...
        (o = this.hooks.callSync(
          ...
          { options: t },
          (r, { options: a }) => (
            window.scrollTo(p({ top: 0, left: 0 }, a)), !0
          )
        )),
      o
    );
  ...
  it = async function (e) {
    ...
    const t = this.hooks.call(
      ...
      { skip: !1 },
      (i, { skip: s }) => {
        if (!s) return this.awaitAnimations({ selector: i.animation.selector });
      }
    );
    await q(),
  ...
  st = async function (e, t) {
    ...
    this.isSameResolvedUrl(S(), i) ||
      (b(i),
      (this.location = v.fromUrl(i)),
      (e.to.url = this.location.url),
      (e.to.hash = this.location.hash)),
      await this.hooks.call("content:replace", e, { page: t }, (s, {}) => {
        if (
        ...
        s.animation.animate &&
          ...
          s.animation.name && this.classes.add(`to-${I(s.animation.name)}`));
      }),
  ...
  nt = function (e) {
    ...
    if (((t = e), !!t?.isSwupPlugin)) {
      if (((e.swup = this), !e._checkRequirements || e._checkRequirements()))
        return (
...
function at(e) {
  if (typeof this.options.resolveUrl != "function")
    return (
      console.warn("[swup] options.resolveUrl expects a callback function."), e
    );
...
class ct {
  ...
  constructor(t = {}) {
    ...
    (this.version = "4.8.2"),
      ...
      (this.cache = new _(this)),
      (this.classes = new z(this)),
      (this.hooks = new G(this)),
      (this.visit = this.createVisit({ to: "" })),
      (this.currentHistoryIndex =
        (i = (s = window.history.state) == null ? void 0 : s.index) != null
          ? i
          : 1),
      this.enable();
  }
  async enable() {
    ...
    (this.clickDelegate = this.delegateEvent(i, "click", this.handleLinkClick)),
    ...
    for (const [s, n] of Object.entries(this.options.hooks)) {
      const [o, r] = this.hooks.parseName(s);
      this.hooks.on(o, n, r);
    }
    ((t = window.history.state) == null ? void 0 : t.source) !== "swup" &&
      b(null, { index: this.currentHistoryIndex }),
      await q(),
  ...
  shouldIgnoreVisit(t, { el: i, event: s } = {}) {
    const { origin: n, url: o, hash: r } = v.fromUrl(t);
    return (
      n !== window.location.origin ||
      !(!i || !this.triggerWillOpenNewWindow(i)) ||
      !!this.options.ignoreVisit(o + r, { el: i, event: s })
    );
  }
  handleLinkClick(t) {
    const i = t.delegateTarget,
      { href: s, url: n, hash: o } = v.fromElement(i);
    if (this.shouldIgnoreVisit(s, { el: i, event: t })) return;
    if (this.navigating && n === this.visit.to.url)
      return void t.preventDefault();
    const r = this.createVisit({ to: n, hash: o, el: i, event: t });
    t.metaKey || t.ctrlKey || t.shiftKey || t.altKey
      ? this.hooks.callSync("link:newtab", r, { href: s })
      : t.button === 0 &&
        this.hooks.callSync("link:click", r, { el: i, event: t }, () => {
          var a;
          const l = (a = r.from.url) != null ? a : "";
          t.preventDefault(),
            n && n !== l
              ? this.isSameResolvedUrl(n, l) || this.performNavigation(r)
              : o
              ? this.hooks.callSync("link:anchor", r, { hash: o }, () => {
                  b(n + o), this.scrollToContent(r);
                })
              : this.hooks.callSync("link:self", r, void 0, () => {
                  this.options.linkToSelf === "navigate"
                    ? this.performNavigation(r)
                    : (b(n), this.scrollToContent(r));
                });
        });
  }
  handlePopState(t) {
    ...
    if (
      ...
      this.isSameResolvedUrl(S(), this.location.url)
    )
      return;
    const { url: a, hash: l } = v.fromUrl(r),
      c = this.createVisit({ to: a, hash: l, event: t });
    c.history.popstate = !0;
    const h = (n = (o = t.state) == null ? void 0 : o.index) != null ? n : 0;
    h &&
